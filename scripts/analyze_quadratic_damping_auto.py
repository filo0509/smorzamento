...
def analyze_oscillation_data(max_time=None, plot_interim=True):
    """
    Analyze the light damping disco forato data file.
    
    Parameters:
    max_time: maximum time to include in analysis (if None, use all data)
    plot_interim: whether to plot intermediate steps
    """
    # Automatically select the light damping file
    ...
    try:
        ...
        plt.title('Quadratic Damping Amplitude Decay Analysis (Light Damping)')
        plt.xlabel('Time (s)')
        ...
        plt.title('Position with Quadratic Damping Envelope (Light Damping)')
        plt.xlabel('Time (s)')
        ...
        # Create a closeup of the early oscillations
        early_time = 20  # seconds
        early_idx = t < early_time
        
        plt.figure(figsize=(12, 6))
        plt.plot(t[early_idx], x_centered[early_idx], 'b-', label='Position Data')
        
        # Add envelope to early section
        t_early = t_env[t_env < early_time]
        env_early = env_quad[:len(t_early)]
        
        plt.plot(t_early, env_early, 'r-', linewidth=2, label='Quadratic Damping Envelope')
        plt.plot(t_early, -env_early, 'r-', linewidth=2)
        
        plt.title('Early Position Data with Quadratic Damping Envelope')
        plt.xlabel('Time (s)')
        plt.ylabel('Position (centered)')
        plt.grid(True)
        plt.legend()
        plt.savefig('results/plots/quadratic_damping_long_early_oscillations.png')
        plt.show()
...
if __name__ == "__main__":
    try:
        ...
        # For users with limited computing power or if interested only in early behavior
        print("\nAdditional analysis with only first 30 seconds of data:")
        analyze_oscillation_data(max_time=30)
    except Exception as e:
